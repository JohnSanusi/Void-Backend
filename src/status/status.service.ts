...
export class StatusService {
  ...
  async createStatus(
    userId: string,
    data: { mediaUrl: string; type: string },
  ...
  async viewStatus(statusId: string, userId: string): Promise<void> {
    ...
    const alreadyViewed = status.viewers.list.some(
      (v) => v.userId.toString() === userId,
    );
    if (!alreadyViewed) {
      // Capping viewer list to 100 for record keeping; count remains accurate
      const updateQuery: {
        $inc: { 'viewers.count': number };
        $push?: { 'viewers.list': { userId: Types.ObjectId; seenAt: Date } };
      } = {
        $inc: { 'viewers.count': 1 },
      };

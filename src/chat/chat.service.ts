...
export class ChatService {
  ...
  async saveMessage(data: {
    ...
    if (conversation.type === 'private') {
      ...
      if (recipientId) {
        const canMessage = await this.usersService.canMessage(
          data.senderId,
          recipientId.toString(),
        );
        if (!canMessage) {
          throw new BadRequestException(
            'Cannot send message: You are blocked or have blocked this user.',
          );
        }
      }
    }
  ...
  async pinConversation(conversationId: string, userId: string): Promise<void> {
    await this.conversationModel.updateOne(
      {
        _id: conversationId,
        'pinned.userId': { $ne: new Types.ObjectId(userId) },
      },
      {
        $push: {
          pinned: { userId: new Types.ObjectId(userId), pinnedAt: new Date() },
        },
      },
    );
  }
      ...
      { _id: conversationId },
      {
        $push: {
          muted: {
            ...
            mutedUntil: mutedUntil || new Date(2099, 0, 1),
          },
        },
      },
    );
  ...
  async markAsUnread(conversationId: string, userId: string): Promise<void> {
    ...
    await this.conversationModel.updateOne(
      {
        _id: conversationId,
        'unreadMarked.userId': { $ne: new Types.ObjectId(userId) },
      },
  ...
  async markAsRead(
    ...
    const unreadIndex = conversation.unreadMarked?.findIndex(

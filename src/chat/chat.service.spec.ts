...
describe('ChatService', () => {
  ...
  beforeEach(async () => {
    ...
    convModel = module.get<Model<ConversationDocument>>(
  ...
  describe('findOrCreatePrivateConversation', () => {
    it('should return existing conversation if it exists', async () => {
      ...
      jest
        ...
        .mockResolvedValue(mockConversation as unknown as ConversationDocument);
  ...
  describe('getMessages', () => {
    it('should call messageModel.find with correct query', async () => {
      ...
      const findSpy = jest
        .spyOn(msgModel, 'find')
        .mockReturnValue(
          mockFind as unknown as ReturnType<typeof msgModel.find>,
        );

      await service.getMessages(convId);

      expect(findSpy).toHaveBeenCalledWith(
        expect.objectContaining({ conversationId: new Types.ObjectId(convId) }),
      );
    });
  });
});

...
export class SettingsController {
  ...
  async updatePrivacy(@Req() req, @Body() dto: UpdatePrivacyDto) {
    return this.settingsService.updatePrivacyPartial(
      (req as RequestWithUser).user.userId,
      dto,
    );
  }
  ...
  async updateMedia(@Req() req, @Body() dto: UpdateMediaDto) {
    return this.settingsService.updateMedia(
      (req as RequestWithUser).user.userId,
      dto,
    );
  }
  ...
  async blockUser(@Req() req, @Body() dto: BlockUserDto) {
    return this.settingsService.blockUser(
      (req as RequestWithUser).user.userId,
      dto.userId,
    );
  }
  ...
  async unblockUser(@Req() req, @Param('id') id: string) {
    return this.settingsService.unblockUser(
      (req as RequestWithUser).user.userId,
      id,
    );
  }
  ...
  async getBlockedUsers(@Req() req) {
    return this.settingsService.getBlockedUsers(
      (req as RequestWithUser).user.userId,
    );
  }
  ...
  async downloadData(@Req() req) {
    return this.settingsService.downloadUserData(
      (req as RequestWithUser).user.userId,
    );
  }
}

    ...
    describe('followUser', () => {
        it('should follow a public user successfully', async () => {
            ...
            (userModel.findById as jest.Mock).mockImplementation((id) => {
                if (id === user1Id) return { select: jest.fn().mockReturnValue({ lean: jest.fn().mockResolvedValue(blockedUsersMock) }) }; // Current user checks target
                if (id === user2Id) return { select: jest.fn().mockReturnValue({ lean: jest.fn().mockResolvedValue(blockedUsersMock) }), ...targetUser }; // Target user found + checked
                return { select: jest.fn().mockReturnThis(), lean: jest.fn(), ...targetUser };
            });
        ...
        it('should throw error if blocked', async () => {
            ...
            userModel.findById.mockImplementation((id) => {
                if (id === user2Id) {
                    ...
                    return {
                        select: jest.fn().mockReturnValue({
                            lean: jest.fn().mockResolvedValue({ blockedUsers: [new Types.ObjectId(user1Id)] })
                        }),
                        then: (resolve) => resolve(targetUser)
                    }
                }
            ...
            await expect(service.followUser(user1Id, user2Id)).rejects.toThrow(BadRequestException);
        });
    });

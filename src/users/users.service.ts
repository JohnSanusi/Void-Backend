...
export class UsersService {
    ...
    async isUserBlocked(userId: string, targetId: string): Promise<boolean> {
        ...
        return user?.blockedUsers?.some(id => id.toString() === targetId.toString()) || false;
    }
    ...
    async canMessage(senderId: string, recipientId: string): Promise<boolean> {
        ...
        const recipientBlockedSender = recipient.blockedUsers?.some(id => id.toString() === senderId.toString());
    ...
    async unfollowUser(currentUserId: string, targetUserId: string) {
        ...
        try {
            ...
            const wasFollowing = await this.userModel.findOne({ _id: targetUserId, followers: new Types.ObjectId(currentUserId) });

            await this.userModel.findByIdAndUpdate(targetUserId, {
                $pull: { followers: new Types.ObjectId(currentUserId), followRequests: new Types.ObjectId(currentUserId) }
            }, { session });

            if (wasFollowing) {
                await this.userModel.findByIdAndUpdate(targetUserId, { $inc: { followersCount: -1 } }, { session });
                await this.userModel.findByIdAndUpdate(currentUserId, { $pull: { following: new Types.ObjectId(targetUserId) }, $inc: { followingCount: -1 } }, { session });
            }
    ...
    async handleFollowRequest(currentUserId: string, requesterId: string, accept: boolean) {
    ...
    async getProfile(viewerId: string, profileId: string) {
        ...
        const isFollowing = profile.followers.some(id => id.toString() === viewerId);
        const isRequested = profile.followRequests.some(id => id.toString() === viewerId);
        ...
        let canViewContent = isMe || !profile.isPrivate || isFollowing;
    ...
    async updateProfile(userId: string, data: any) {
        if (data.username) {
            const existing = await this.userModel.findOne({ username: data.username, _id: { $ne: userId } });
